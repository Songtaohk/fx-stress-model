...
const metricDocs: Record<StressMetricKey, MetricDoc> = {
  ...
  nominal10yYield: { meaning: { zh: "10年期政府债券名义收益率，衡量长期利率环境。", en: "Nominal 10-year government bond yield, measuring long-end rates." }, formula: "Annual average 10Y government bond yield", sources: [{ label: "ECB Data Portal", url: "https://data.ecb.europa.eu/" }, { label: "FRED", url: "https://fred.stlouisfed.org/" }, { label: "OECD Data", url: "https://data-explorer.oecd.org/" }, { label: "TPEx Taiwan", url: "https://www.tpex.org.tw/en-us/bond/info/quote/yield.html" }] },
...
function App() {
  ...
  const renderRefreshStep = (title: string, step: typeof stressRefreshSummary.fx | typeof stressRefreshSummary.macro) => {
    const visibleItems = step.items.length > 0
      ? step.items
      : [language === "zh" ? "今日累计未发现任何新数据公布。" : "No new data has been found today."];
    return <article className="refreshCard"><strong>{title}</strong><p>{step.message[language]}</p><ul>{visibleItems.map((item) => <li key={item}>{item}</li>)}</ul>{step.errors.length > 0 && <div className="refreshWarnings"><b>{language === "zh" ? "数据源提示" : "Source warnings"}</b><ul>{step.errors.map((item) => <li key={item}>{item}</li>)}</ul></div>}</article>;
  };

  const renderAutoUpdateRules = () => {
    const refreshStatus = String(stressRefreshSummary.status);
    const statusLabel = refreshStatus === "success-no-new-data"
    ...
    return <section className="readmeSection refreshSummarySection"><h3>{language === "zh" ? "自动更新规则" : "Automatic Update Rules"}</h3><p>{language === "zh" ? "每日刷新完成后，本区显示当天累计的刷新结果：正式入库数据、年内滚动预览值，以及需要人工复核的数据源提示。当天重复刷新不会清空已经记录的累计更新；跨到新日期后重新累计。" : "After each daily refresh, this section shows the cumulative same-day refresh result: archived data updates, rolling current-year preview values, and source warnings that need review. Repeated refreshes on the same day do not clear accumulated updates; accumulation restarts on a new date."}</p><div className="refreshSummaryHeader"><span>{language === "zh" ? "更新时间" : "Refresh time"}：{stressRefreshSummary.refreshedDate}</span><span className={`refreshStatus ${refreshStatus}`}>{statusLabel}</span></div><p className="refreshHeadline">{stressRefreshSummary.headline[language]}</p><div className="refreshGrid">{renderRefreshStep(language === "zh" ? "汇率数据刷新" : "FX refresh", stressRefreshSummary.fx)}{renderRefreshStep(language === "zh" ? "宏观数据刷新" : "Macro refresh", stressRefreshSummary.macro)}</div></section>;
  };
