...
- 高频指标（CPI、10年期名义收益率、实际政策利率、实际政策利率 Z-score）按月度或月末观测值生成年度或年内滚动均值；当前年份可以作为滚动年内值展示和更新，但已结束年份必须有 12 个有效月度/月末观测值，或官方年度值，才允许写入为历史数据。
- 未最终确认但可展示的值写入 `stressPreviewData`，只作为临时预览层：图表中用虚线和空心点展示，标注 YTD / provisional，不参与历史存档截止年份，也不锁定下一轮自动更新。
